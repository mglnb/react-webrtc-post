...
class PeerConnection {
   ...
   * @param {String} type
   */
  static getUserMedia(type = "offer") {
    if (type === "offer") {
  ...
  bindEvents() {
    ...
    this.peerConnection.onsignalingstatechange = e => {
      log(`---- estado do servidor: ${e.target.signalingState}`);
    };
    /**
    ...
    if (this.dataChannel) {
      this.dataChannel.onmessage = event => {
        log(
          "ondatachannelmessagelocal: " + JSON.stringify(event),
          "log",
          "purple"
        );
        log(
          "remoteStreams: " + this.peerConnection.getRemoteStreams(),
          "log",
          "purple"
        );
        console.log(this.peerConnection.getRemoteStreams());
        Chat.appendMsg({
          username: "teste",
          message: event.data
        });
  ...
  bindChannelEvent() {
    this.peerConnection.ondatachannel = e => {
      ...
      this.dataChannel.onmessage = event => {
        log(
          "ondatachannelmessageRemote: " + JSON.stringify(event),
          "log",
          "purple"
        );
        log(
          "remoteStreams: " +
            JSON.stringify(this.peerConnection.getRemoteStreams()),
          "log",
          "purple"
        );
        Chat.appendMsg({
  ...
  messageRtcClient(data) {
    ...
    this.peerConnection
      .setRemoteDescription(data.data)
      .then(() =>
        Video.addRemoteStream(this.peerConnection.getRemoteStreams()[0])
      )
      .then(() =>
  ...
  iceCandidateClient(data) {
    ...
    if (!this.peerConnection.remoteDescription.type) {
      this.peerConnection
        ...
        .catch(err =>
          log(`${err} ${JSON.stringify(data.ice, null, "\t")}`, "error")
        );
    }
  }
  /**
